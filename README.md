...
- **Live Monitor** — current test tracking by machine and position, with an occupation breaks viewer
...
The clock icon button on each position card opens the **Occupation Breaks** dialog for that machine/position slot. It is purely informative — the app never writes to any Excel file. The user copies the detected break text and pastes it into their own occupation spreadsheet, so any data validation (dropdowns, etc.) already in that spreadsheet is untouched.

### What it shows

For each selected date, a preview row shows the break intervals detected from the parsed log: any continuous period where **speed == 0** becomes one `Break:HH:MM-HH:MM` entry. Multiple breaks in the same day are joined with a single space, e.g. `Break:09:00-09:30 Break:14:00-14:15`.

### Usage

1. Click the **clock icon** on a position card (visible when a test is loaded).
2. Select a date range with the date picker.
3. Each date row shows its break text with a small clipboard icon next to it — click it to copy just that day's text.
4. Click **Copy all days** to copy every day in the range at once, one line per day, ready to paste.
...
The app detects Windows automatically and reads logs from `Z:\prstruh\ctend_pt`. No environment variables needed — the drive must be mapped.
  ...
  features/
    monitor/
      ...
      auto_refresh/
        ...
        callbacks.py
        layout.py
      occupation/                 ← Occupation Breaks feature (read-only, copy-to-clipboard)
        __init__.py
        excel_writer.py           ← pure logic: break detection, date range helper
        layout.py                 ← clock button + occupation modal HTML
        callbacks.py              ← toggle modal, preview + clipboard content
    navigation/
      callbacks.py
    analysis/
      layout.py
